...
| `urls:read` | View URL details and list | `GET /api/v1/urls`, `GET /api/v1/stats*`, `GET /api/v1/export*` |
| `urls:manage` | Update and delete URLs | `PATCH/DELETE /api/v1/urls/*` |
| `stats:read` | Access analytics data | `GET /api/v1/stats*`, `GET /api/v1/export*` |
...
```bash
curl -X GET "https://spoo.me/api/v1/stats" \
  -H "Authorization: Bearer spoo_AbCdEfGhIjKlMnOpQrStUvWxYz"
```
...
API keys grant **authenticated rate limits**, which are significantly higher than anonymous limits. The numbers below are the default API tier; some endpoints use their own limits, see [Rate Limits](/rate-limits) for the per-endpoint breakdown:
...
<CardGroup cols={2}>
  <Card title="With API Key" icon="bolt" color="#10B981">
    ...
    - Full analytics and export for your links
  </Card>
  <Card title="Without API Key" icon="turtle" color="#EF4444">
    - **20 requests/minute**
    - **200 requests/day**
    - Cannot manage URLs
    - Stats via the public per-link endpoint only
  </Card>
</CardGroup>
