...
<Step title="Configure Environment Variables">
    ...
    The full public URL of your deployment, including the scheme (e.g., `https://your-project.vercel.app`). The app derives its default short-link domain from this value.
    </ParamField>

    <Warning>
    `APP_URL` must match the domain you actually serve the app from. Requests arriving on any other hostname are treated as an unknown custom domain and answered with a 404, so a missing or wrong `APP_URL` makes the entire deployment appear broken.
    </Warning>
