...
<Note>
The public stats endpoint only serves links whose stats are public; links with private stats respond with a 404. Password-protected links require a `POST` request with the password in the JSON body. For account-wide analytics and your own private links, use the authenticated `GET /api/v1/stats` endpoints with [API key authentication](/api-keys).
</Note>
