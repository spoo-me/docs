...
Retrieve public analytics for your shortened URL:

<CodeGroup>

```bash cURL
curl -X GET "https://spoo.me/api/v1/public/stats/abc123"
```

```python Python
import requests

response = requests.get("https://spoo.me/api/v1/public/stats/abc123")
...
<Note>
The public stats endpoint only serves links whose stats are public; links with private stats respond with a 404. Password-protected links require a `POST` request with the password in the JSON body. For account-wide analytics, use the authenticated `GET /api/v1/stats`; for a single link, including your private ones, use `GET /api/v1/stats/links/{url_id}`. Both require [API key authentication](/api-keys).
</Note>
